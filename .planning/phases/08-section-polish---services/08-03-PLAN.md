...
must_haves:
  truths:
    ...
    - "NuxtImg component uses format='webp' for optimal image optimization"
    - "Images display correctly without impacting micro-interaction performance"
  artifacts:
    - path: "pages/services/[slug].vue"
      provides: "Service detail page with verified hero image mappings using NuxtImg with loading='eager'"
      min_lines: 350
  key_links:
    ...
    - from: "pages/services/[slug].vue"
      ...
      via: "Reduced motion support applies to page transitions, not static images"
      pattern: "@media.*prefers-reduced-motion"
---

<objective>
Verify and optimize service hero image mappings using Phase 3 migrated images, ensure all image paths exist and load correctly, and confirm WebP format with responsive srcset usage. Note: Image loading has no impact on micro-interactions (images load independently of hover/focus states).

Purpose: Service detail pages use hero images from the Phase 3 project image migration. This task verifies all mappings point to existing files, confirms proper image optimization (WebP format), and ensures fallback behavior works for any unmapped services. No new image downloads needed - all images exist from Phase 3. Image verification is separate from micro-interaction functionality.

Output: Verified service hero image mappings with all paths confirmed existing, proper NuxtImg usage with WebP format and eager loading, working fallback for unmapped services, and confirmation that images don't impact micro-interaction performance.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    1. Check each mapped image file exists:
       Run: ls -la /Users/austinlaw/Desktop/Sites/vp-eng-nuxt/public/images/projects/*.webp
    ...
    4. Confirm fallback image exists:
       Run: ls -la /Users/austinlaw/Desktop/Sites/vp-eng-nuxt/public/images/hero/home-header*.webp
    ...
    Note: Image verification is independent of micro-interactions. Images load via NuxtImg with loading="eager" for above-fold content, which doesn't interfere with hover/focus states defined in CSS.
  </action>
...
<task type="auto">
  ...
  <action>
    ...
    6. Note: loading="eager" is correct for above-fold hero images. This ensures immediate loading without delay, which is appropriate for LCP (Largest Contentful Paint) optimization. Micro-interactions (hover/focus states) are CSS-based and run independently of image loading.

    7. Note: Reduced motion support in main.css applies to CSS transitions/transforms, not image loading behavior. Images load normally regardless of motion preferences.
  </action>
  ...
  <done>NuxtImg component verified with all attributes: src binding, alt text, object-cover class, format="webp", loading="eager", width=1920, height=600. @nuxt/image module confirmed in package.json from prior phases. Image loading (eager) doesn't interfere with CSS-based micro-interactions.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    4. Verify visual appearance:
       ...
       - Hover effects on buttons/links work (micro-interactions independent of images)

    5. If any 404s found, check serviceHeroImages mapping and fix the path.

    6. Note: This is verification only - the mapping was created in Plan 02. This task confirms the implementation works.

    7. Note: Image performance and micro-interaction performance are independent. Images load via NuxtImg module, hover/focus states are CSS transitions. Images don't block or interfere with micro-interactions.
  </action>
  <verify>npm run dev, visit each of the 10 service detail pages, check DevTools Network tab for 404 errors (should be none), verify images load as WebP, visually verify hero images display with overlay, verify hover/focus states work regardless of image load status</verify>
  <done>All 10 service detail pages load successfully, no 404 errors for hero images, all images load as WebP format, hero images display with proper aspect ratio and dark overlay, text remains readable over all hero backgrounds, micro-interactions (hover/focus) work correctly and are independent of image loading</done>
</task>
...
9. Micro-interactions work independently of image loading
</verification>
...
- Image loading doesn't interfere with micro-interaction performance
</success_criteria>
