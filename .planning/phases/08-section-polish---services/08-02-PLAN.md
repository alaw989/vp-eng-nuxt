...
must_haves:
  truths:
    ...
    - "Hero section CTA buttons use hover:-translate-y-0.5 with transition-all duration-300"
    - "Related services cards use hover:-translate-y-1 lift effect from ServiceCard component"
    - "All focusable elements have focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
    - "Back to Services link uses hover:text-white hover:underline transition-colors duration-300"
    - "Reduced motion users see color transitions without transform animations"
  artifacts:
    - path: "pages/services/[slug].vue"
      provides: "Service detail page with hero image, process section, dynamic related services, and micro-interaction hover/focus states"
      min_lines: 350
  key_links:
    ...
    - from: "pages/services/[slug].vue"
      ...
      pattern: "hover\:-translate-y.*@media.*prefers-reduced-motion"
---

<objective>
Enhance service detail page with hero image background using Phase 3 project images, add "How This Service Works" 4-step process section, implement dynamic "Related Services" replacing static "Other Services", and apply Phase 18 micro-interaction patterns.

Purpose: Service detail pages need visual parity with Projects pages. Hero images using related project photos create thematic connection. The generic 4-step process section (same for all services) helps users understand the service workflow. Dynamic related services (same category) improve discoverability over hardcoded services. Micro-interactions enhance usability with consistent hover states, focus indicators, and reduced motion support per Phase 18 patterns.

Output: Enhanced service detail page with hero image background, overlay treatment, 4-step process section, dynamic related services based on category matching, and consistent hover/focus states per Phase 18 patterns.
</objective>
...
<task type="auto">
  <name>Add hero image background to service detail page header with micro-interactions</name>
  <files>pages/services/[slug].vue</files>
  <action>
    Modify pages/services/[slug].vue to add hero image background with Phase 18 micro-interaction patterns:
    ...
    2. Modify Page Header section (lines 32-56) to include hero background with micro-interactions:
       ...
       <AppSection bg-color="primary-dark" padding="lg" class="relative overflow-hidden">
         ...
         <div class="container text-white relative z-10">
           ...
           <NuxtLink to="/services" class="inline-flex items-center gap-2 text-white/80 hover:text-white hover:underline transition-colors duration-300 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded px-1 mb-6">
    ...
    - Back link uses hover:text-white hover:underline transition-colors duration-300
    - Back link has focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 for keyboard accessibility
    - CTA buttons added later will use hover:-translate-y-0.5 with transition-all duration-300
    - Reduced motion support from main.css disables transforms while preserving color transitions
  </action>
  <verify>npm run dev, navigate to http://localhost:3000/services/structural-steel-design, verify hero image displays with dark overlay, verify white text remains readable, verify icon box shows with white/10 background, verify back link has hover effect and focus ring, test other service slugs to confirm different hero images</verify>
  <done>Hero image displays as full-width background, dark overlay gradient (from-black/70 via-black/50 to-black/70) ensures text readability, white text displays over overlay, icon box shows with white/10 background and backdrop-blur, back link has hover:text-white hover:underline with duration-300 timing and focus-visible rings, fallback image loads for unmapped services</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    Note: Process steps are visual/informational (not interactive), so no hover effects needed.
  </action>
...
<task type="auto">
  <name>Replace static 'Other Services' with dynamic 'Related Services' with micro-interactions</name>
  <files>pages/services/[slug].vue</files>
  <action>
    Replace static "Other Services" with dynamic "Related Services" based on category with Phase 18 micro-interaction patterns:
    ...
    4. Update template section (lines 149-180), replace "Other Services" with:
       ...
       <AppSection v-if="relatedServices.length > 0" bg-color="neutral-100" animate-on-scroll>
         ...
         <div class="text-center mt-12">
           <NuxtLink
             to="/services"
             class="inline-flex items-center gap-2 px-6 py-3 border-2 border-primary text-primary rounded-lg font-semibold hover:bg-primary hover:text-dark hover:-translate-y-0.5 transition-all duration-300 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
           >
             View All Services
    ...
    Micro-interaction notes:
    - ServiceCard component already has hover:-translate-y-1 lift effect from Phase 18
    - "View All Services" button uses hover:bg-primary hover:text-dark hover:-translate-y-0.5 transition-all duration-300
    - Button has focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 for keyboard accessibility
    - Reduced motion support from main.css disables transforms while preserving color transitions
  </action>
  <verify>npm run dev, navigate to structural-steel-design, verify related services show other structural services (concrete, masonry, wood, foundation), navigate to inspection-services, verify related services empty (only one in inspection category), verify "View All Services" button has hover lift effect and focus ring</verify>
  <done>Related Services section displays up to 3 services from same category, current service excluded from results, section uses ServiceCard component with hover:-translate-y-1 lift effect, heading changed from "Other Services" to "Related Services", description updated to "Explore other services that may fit your project needs", View All Services button displays with hover:-translate-y-0.5 lift effect and duration-300 timing, focus-visible rings on all interactive elements</done>
</task>
...
6. Back to Services link has hover:text-white hover:underline with duration-300 timing
7. "How This Service Works" section displays 4-step process
8. Process steps are: Consultation, Design, Review, Support
9. Connecting lines display between steps on desktop
10. "Related Services" section displays services from same category
11. Current service is excluded from related services results
12. ServiceCard component has hover:-translate-y-1 lift effect
13. "View All Services" button has hover:-translate-y-0.5 lift effect
14. All interactive elements have focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2
15. Reduced motion is respected (transforms disabled, colors preserved)
</verification>
...
- All buttons use 300ms duration timing with hover lift effects
